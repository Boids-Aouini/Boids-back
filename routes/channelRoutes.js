...
router.get('/getPosts/:server_id/:channel_id', verify, async (req, res) => {
    ...
    Con.query('SELECT Channels_Posts.id, Channels_Posts.user_id, Channels_Posts.post, Channels_Posts.isHidden, Users.firstname, Users.lastname FROM Channels_Posts INNER JOIN Users ON Channels_Posts.channel_id = (?) AND (Users.id IN (SELECT leader_id FROM Servers WHERE id = (?)) OR Users.id = Channels_Posts.user_id)', [channel_id, server_id],
        (error, posts) => {
